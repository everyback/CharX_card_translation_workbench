    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div className="settings-dialog" role="dialog" aria-modal="true" aria-labelledby="settings-title">
        ...
        <div className="settings-grid">
          ...
          <label><span>模型请求并发</span><input type="number" min="1" max="8" step="1" value={value.concurrency} onChange={(event) => setValue({ ...value, concurrency: Number(event.target.value) })} /><small>当前配置 {Math.min(8, Math.max(1, Number(value.concurrency) || 1))} 路；正文翻译与阶段 2 共用同一模型请求通道，上限 8 路。</small></label>
          <label><span>每批段落</span><input type="number" min="1" max="8" step="1" value={value.batchItems} onChange={(event) => setValue({ ...value, batchItems: Number(event.target.value) })} /></label>
          <label><span>每批字符</span><input type="number" min="1000" max="40000" step="500" value={value.batchChars} onChange={(event) => setValue({ ...value, batchChars: Number(event.target.value) })} /></label>
