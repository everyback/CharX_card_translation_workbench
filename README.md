...
直接下载 Windows 桌面免安装版即可使用，无需安装 Node.js、Docker 或自行编译：前往 [GitHub Releases](https://github.com/everyback/CharX_card_translation_workbench/releases) 下载。
...
3. **检查概要**：确认卡片格式、角色主体、世界书、脚本、Lua、协议和资源数量符合预期。数量异常时暂停翻译流程。
4. **选择翻译范围**：一般使用默认的“全部翻译”。如果只想处理主体或世界书，可以改用下拉框中的其他范围。
5. **扫描并开始翻译**：点击“扫描卡片”或“扫描字段”，确认待翻译内容后点击“开始翻译”。大卡建议从较低并发和较小批次开始。
6. **处理失败任务**：到“任务”页查看失败原因。修正配置或补充人工译文后再重试；结构性错误应先修正后再重试。
7. **选择审核方式**：翻译完成后，已确认的内容可使用“一键通过全部”；需要复核的内容进入“审核”。“一键通过”仅处理已有译文，不处理未翻译或失败项。
...
遇到陌生协议或脚本时，在“协议”和“引用”页确认代码使用的文字，再决定是否通过；导出判断应同时依据结构校验和译文质量。
...
默认情况下，变量名、ID、路径、URL、正则、Lua 结构、协议外壳和按钮触发器不会被改写。遇到不熟悉的协议，应先在“协议”和“审核”页面完成确认，再决定是否批量通过。
...
随后重新执行 `docker compose -f docker/compose.yml up -d`，并访问 `http://127.0.0.1:18880/`。端口映射应保持在回环地址，不应设置为 `0.0.0.0` 或直接暴露到公网。
...
推送到 GitHub 后，`.github/workflows/desktop-release.yml` 会自动编译这两种 EXE，并将构建产物上传到 Actions 运行记录。推送到 `master` 的普通提交会自动更新一个可下载的 `CardLoom Translate Nightly` 预发布版本；正式版本应先同步推进 `package.json` 与 `package-lock.json` 的版本号，再推送同号 `v` 前缀标签（例如 `v0.1.3`）。工作流会校验标签与包版本一致，创建独立的正式 GitHub Release，并永久保留带版本号的安装包；旧版本不会被 Nightly 覆盖。
...
端口、数据目录等部署参数放在仓库根目录的 `.env` 里；API Base URL、模型、API Key、语言、并发、批次和图片设置只能从页面左侧底部的“模型设置”完成。`.env` 只用于部署参数，不接受 `TRANSLATION_*` 或 `IMAGE_EDIT_*` 配置，也不会把任何模型设置写入 SQLite。页面保存的模型配置会写入本地 SQLite；API Key 由使用者自行配置且仅由后端读取，浏览器不会展示密钥内容。

### 官方接口示例

下面只列出公开的官方接口示例，不代表项目当前配置。模型名称和 API Key 应在页面“模型设置”中自行配置：
...
页面参数可按下面的值开始，再根据卡片大小和模型服务商限制调整：
...
正文翻译与阶段 2 共用同一模型请求通道。并发和批次参数只校验为正整数，没有产品固定上限；数值越大，内存占用、接口费用、上下文长度和限流风险越高，请根据模型服务商实际能力自行调整。

## 数据与安全

- 本地 Node.js 默认把 SQLite、上传卡片、草稿和缓存放在 `data/`；Docker 使用 Compose 命名卷。两种方式默认不是同一份数据，切换时看不到另一边的项目是正常的。
- 当前没有登录功能，默认只适合本机使用。服务应保持 `127.0.0.1` 绑定；远程访问必须通过可信反向代理提供认证、TLS 和访问控制。
- 升级、切换部署方式、批量导出或清理数据前，应备份重要项目。尤其注意：`docker compose down -v` 会删除 Compose 数据卷。
- 陌生卡片导出前，应再次检查 Lua、正则、协议、世界书触发词和资源引用。工作台的保护规则仅作辅助，不能替代人工确认。

## 贡献

格式兼容、解析或翻译保护问题请提交 Issue；附上脱敏后的日志和最小复现样例有助于定位。提交代码前需：

1. 模型 Key、导入卡片、SQLite、备份和日志不得进入提交。
